...
void considerTouchInput(int lx, int ly){
  ...
  if (lx > 0 && ly > 0 ){
    ...
    if (CURRENT_SCREEN == 0){   // captures touches on drawGPSScreen()
      ...
      if (lx > 0 && lx < 100 && ly > 0 && ly < 100) //(lx > 0 && lx < 55 && ly > 0 && ly < 55)
      {
  ...
  if (lx > 0 && lx < 100 && ly > 100 && ly < 200) //(lx > 0 && lx < 55 && ly > 0 && ly < 55)
      {
       // Load the Alignment screen
         Serial.println("Entering alignment mode");
         tft.drawRect(0,0,55,55, btn_l_border);
         CURRENT_SCREEN =3;
         GPS_iterrations = 0;
         IS_IN_OPERATION = false;
    ...
    }else if (CURRENT_SCREEN == 8){   // captures touches on drawFilterScreen()
       if (lx > 181 && lx < 238 && ly > 5 && ly < 35){
       // BTN <Back pressed
         drawLoadScreen();
        }
       if (lx > 5 && lx < 79 && ly > 65 && ly < 95){
       // Magnitude All filter selected
         N_magnitude_threshold = 100;
         Serial.print("N_magnitude_threshold= ");
         Serial.println(N_magnitude_threshold);
         drawFilterScreen();
        }
       if (lx > 83 && lx < 156 && ly > 65 && ly < 95){
       // Magnitude > 4 filtering selected
         N_magnitude_threshold = 6;
         Serial.print("N_magnitude_threshold= ");
         Serial.println(N_magnitude_threshold);
         drawFilterScreen();
        }
       if (lx > 161 && lx < 234 && ly > 65 && ly < 95){
       // Magnitude > 6 filtering selected
         N_magnitude_threshold = 4;
         Serial.print("N_magnitude_threshold= ");
         Serial.println(N_magnitude_threshold);
         drawFilterScreen();
        }
       ...
       if (lx > 128 && lx < 240 && ly > 360 && ly < 400){
          ...
          if (ALLIGN_STEP == 1){
              if (ALLIGN_TYPE == 1){
                IS_TRACKING = false;
                IS_IN_OPERATION = true;
                OBJECT_NAME = ""; //Ensure correct slewing to message when alignment is completed
                drawMainScreen();
  ...
  //**************************************************************
    ...
    if (CURRENT_SCREEN == 0){ 
      if (last_button == 1){
          ...
          Serial.print("STP_FWD_RA = ");
          Serial.println(STP_FWD_RA);
          Serial.print("STP_BACK_RA = ");
          Serial.println(STP_BACK_RA);          
          CURRENT_SCREEN = 1;
          drawClockScreen();
      }
        ...
        if (last_button == 2 && IS_BT_MODE_ON == false){
         ...
         N_selected_object_index = -1; //set CP for currently selected for indicate visible objects
         N_selected_object_set = -1;   //set CP for currently selected for indicate visible objects
        }
        ...
        if (autoguiding_active){
          autoguiding_active = false; //user turned of autoguiding
          IS_TRACKING = true; //this sets up the below code to disable tracking
        }
    ...
    //delay(120); //LCD touchscreen debounce in lcd backpack version
  }
}
