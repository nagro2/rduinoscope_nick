...
int N_selected_object_index = -2; //hold currently selected for indicate visible objects
int N_selected_object_set = -2;   //hold currently selected for indicate visible objects
String N_visible_magnitude; //for indicate visible objects
String N_object_size; //for indicate visible objects
String N_Object_type; //
bool N_show_magnsize = true; //whether to show magnitude and size in selector buttons
bool N_show_isvisible = true; //whether to show in selector screen whether object is visible
float ALT_threshold = 0; // minimum altitude to use when determining whether an object is visible, 0 = horizon
String N_object_filter = "Vis"; //filtering of objects during select, can use "All" if preferred
float N_magnitude_threshold = 100; //use for filtering objects by magnitude
bool stepsmode_r1 = false; //use in RA joystick control
int DST_mode = 2; //DST = Summer, 0 = no DST, 1 = Europe, 2 = USA

////////////////////////////////////////////////
//variables for comaptibility with Giacomo storeOptions_SD and loadOptions_SD only
String Fan1_State = "OFF";
String Fan2_State = "OFF";
String TFT_Time = "AL-ON";
String Sound_State = "ON";
String Stepper_State = "ON";
String Mer_Flip_State = "Auto";
String Tracking_Mode = "Celest";
boolean IS_FAN1_ON = false;
boolean IS_FAN2_ON = false;

#define serial_debug       // comment out to deactivate the serial debug mode

//add these to Giacomo options
int loaded_gps_from_SD = 0; //whether last learned gps position is stored on SD

//end variables for comaptibility with Giacomo storeOptions_SD and loadOptions_SD
...
//backp[ack display pin touch map
#define DCLK     6
#define CS       5  
#define DIN      4 
#define DOUT     3
...
#define Pink            0xF81F
#define DarkerGrey      0x496A      //0x59CB     
#define DHTPIN 22 //3
...
DS3231 rtc(SDA, SCL);           // (SDA, SCL) from the RTC board
//DS1307 rtc(SDA, SCL);           // (SDA, SCL) from the RTC board
...
"UMa;Dubhe;11h3.72m;61.75°",  //"UMa;Dubhe;11h4m;61.8°",
"UMa;Megrez;12h15m;57°",
"UMa;Merak;11h1.85m;56.38°", //"UMa;Merak;11h2m;56.4°",
"UMa;Mizar;13h23.9m;54.92°", //"UMa;Mizar;13h24m;54.9°",
...
int STP_FWD_DEC = LOW;
int STP_BACK_DEC = HIGH;
bool RA_reverse = true; //  change to opposite bool to reverse RA direction
bool DEC_reverse = true;// change to opposite bool to reverse DEC direction
...
float OBSERVATION_LONGITUDE = -73.02; // Mount Sinai, NY
float OBSERVATION_LATTITUDE = 40.95; 
float OBSERVATION_ALTITUDE = 10; //760.60; // Sofia, Bulgaria
int TIME_ZONE = -5; // 2; // Sofia, Bulgaria
// .............................................................
...
boolean IS_SOUND_ON = true; // 3/8/20
...
// PIN selection
int speakerOut = DAC1; //23; //2;
//int dht_pin = 3;
// Day/Night mode - daynight_pin was A6;
int RA_STP = 31; //4;
int RA_DIR = 32; //5;
int DEC_STP = 26; //6;
int DEC_DIR = 27; //7;
...
int DEV1 = A2;
int DEV2 = A3;
int TFTBright = 13; //DAC0;
int Joy_SW = 24; //not currently using this
int POWER_DRV8825 = 37; //provides ability to enable/disable stepper motors


#define AUTOGUIDING
bool autoguiding_initialized = false;
bool autoguiding_active = false;
#define pinLED 13

//int calx = 8, caly = 11.59;
//int calx = 2, caly = 11.59;

void setup(void) {

  analogWrite(TFTBright, 128); //nja

  Serial.begin(115200);// Console
  Serial.println("NJA diagnostics");
  ...
  //Serial.begin(115200);// Console
  Serial2.begin(4800); // Initialize GPS communication on PINs: 17 (RX) and 16 (TX) 
  ...
  bool touch_init = ts.begin();  //ADS7873 begin();
...
//  Timer3.start(Clock_Sidereal); // executes the code every 62.329 ms.
  ...
  pinMode(daynight_pin, INPUT_PULLUP); //daynight pin is digital with internal pullup

  //if (analogRead(A6) < 800){
   //if (analogRead(daynight_pin) < 800){
   //if ( (false) ) // Temporarily disable day-night mode
  ...
  tft.println("Enhancements by Nick Agro, Jun 2019");

  bool startup_error = false;

  if (!SD.begin(sd_cs)) {
    tft.println("ERROR: Card failed, or not present, bypassed");
    startup_error = true;
  ...
  drawPic(&StarMaps, 0, 308, 120, 24); //Fix supporters display for backpack lcd by setting to native resolution of bmp
  StarMaps.close();
  delay(200);
  tft.setCursor(5, 300);
  ...
  if (IS_SOUND_ON){
    ...
    delay(100);
    */
    SoundOn(note_cb, 32); //NJA play only the last three notes to make startup faster
    delay(140);
    SoundOn(note_gb, 8);
    delay(50);
    SoundOn(note_f, 48);
...
//          Serial.print(items);
  ...
  //SD.remove("options.txt"); //Uncomment to remove corrupted options file from SD
...
void loop(void) {
 ...
 if ((DELAY_Slew + 1 <= millis()) && (IS_OBJ_FOUND == false)){
  ...
  if (IS_OBJ_FOUND == true){    
      ...
      #ifdef AUTOGUIDING
      if (! autoguiding_initialized){//autoguiding has not been ititialized yet, see if it should be now
        if (SerialUSB){
          autoguiding_initialized = true;
          //autoguiding_active = true;
          SerialUSB.println("INITIALIZED#");
          Serial.println("sent INITIALIZED#");
        }
      ...
      // Always check touchscreen regardless of in autoguide or not
    ...
    Serial.println(tp.y);
      while (tp.z < 600)
      {
        tp = ts.getPoint(); //to remove noise
        delay(100);
      }

      
      int calx = 8, caly = 11;
      tx = (3860 - tp.x) / calx; //NOT SWAPPING X & Y HERE
      ty = (tp.y - 200) / caly;
    ...
    considerTouchInput(ty, tx);//SWAPPING X & Y HERE

      

      if (! autoguiding_active){ //only run rest of main loop if not autoguiding
      ...
      bool joystick_enable = true; //allow joystick disable for development / troubleshooting
      if (joystick_enable){
        
      // y threshold of 460 is due to potentiometer drift to 468. May want to add calibration
      if ((xPosition < 410) || (xPosition > 620) || (yPosition < 410) || (yPosition > 620)){
        IS_MANUAL_MOVE = true; //comment this out if no joystick is connected
        ...
        //NJA fix for releasing ra quickly leaving steps mode R 1
        //which made RA_STP stay on indefinitely
        if (stepsmode_r1){ //if ended manual move in R 1
          stepsmode_r1 = false;
          consider_Manual_Move(419, 511); //step one last time slower than R 1
        }
      }
      ...
      if (((millis()- UPD_T) > 2000)&&(IS_MANUAL_MOVE == false)){
        ...
        //Serial.println("time updates next");
        considerTimeUpdates();
...
void selectOBJECT_M(int index_, int objects) {
    ...
    N_selected_object_index = index_; //use with indicate visible objects
    N_selected_object_set = objects; //use with indicate visible objects
   ...
   if (objects == 0){                                           // I've selected a Messier Object
      ...
      N_Object_type = Messier_Array[index_].substring(i4+1,i5);
      Serial.print("object type= ");
      Serial.println(N_Object_type);
      ...
      N_visible_magnitude = Messier_Array[index_].substring(i5+1,i6);
      Serial.print("visible magnitude= ");
      Serial.println(N_visible_magnitude);
      OBJECT_DETAILS += Messier_Array[index_].substring(i5+1,i6) + " and size of ";
      N_object_size= Messier_Array[index_].substring(i6+1,i7);
      Serial.print("size= ");
      Serial.println(N_object_size);
      ...
      Serial.println(OBJECT_NAME);
      OBJECT_DETAILS = OBJECT_NAME + " is a ";
      N_Object_type =  Treasure_Array[index_].substring(i4+1,i5);
      Serial.print("object type= ");
      Serial.println(N_Object_type);
      ...
      N_visible_magnitude = Treasure_Array[index_].substring(i5+1,i6);
      Serial.print("visible magnitude= ");
      Serial.println(N_visible_magnitude);
      OBJECT_DETAILS += Treasure_Array[index_].substring(i5+1,i6) + " and size of ";
      N_object_size= Treasure_Array[index_].substring(i6+1,i7);
      Serial.print("size= ");
      Serial.println(N_object_size);
    ...
    if (IS_OBJECT_DEC_FOUND == false){    
        if (SLEW_DEC_microsteps >= (DEC_microSteps-DEC_mode_steps) && SLEW_DEC_microsteps <= (DEC_microSteps+DEC_mode_steps)){
              ...
              if (SLEW_DEC_microsteps > DEC_microSteps){
                  digitalWrite(DEC_DIR,STP_BACK_DEC);
                  ...
                  digitalWrite(DEC_DIR,STP_FWD_DEC);
                  //digitalWrite(DEC_STP,HIGH);
                  //digitalWrite(DEC_STP,LOW);
                  //-PIOC->PIO_SODR=(1u<<24);
                  PIOD->PIO_SODR=(1u<<1);
...
void UpdateObservedObjects(){
   ...
   ObservedObjects[Observed_Obj_Count]= OBJECT_NAME+";"+OBJECT_DETAILS+";"+String(rtc.getTimeStr()).substring(0,5)+";"+int(_temp  * 1.8 + 32)+";"+int(_humid)+";"+int(HAHour)+"h "+HAMin+"m;"+int(ALT); //CtoF
...
void DrawButton(int X, int Y, int Width, int Height, String Caption, int16_t BodyColor, int16_t BorderColor, int16_t TextColor, int tSize, String lower_line = ""){
//  TYPE: 0:Solid color, no Frame; 1: Frame Only button; 2: Solid color and Frame button;
   ...
   //NJA display add'l info in buttons
   if (lower_line != ""){
    TX = X + 1 ; // + Width/2 ;
    tft.setCursor((int)TX + 6, (int)TY + 18);
    tft.setTextSize(1);
    tft.println(lower_line);
   }
}
