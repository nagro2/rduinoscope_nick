...
void considerBTCommands(){
      ...
      if (BT_COMMAND_STR.indexOf("gotoHome")>0 || BT_COMMAND_STR.indexOf("gotohome")>0 || BT_COMMAND_STR.indexOf("gh")>0)  {
         Serial.println("gotoHome command received");
         IS_TRACKING = false;
         Timer3.stop();
         OBJECT_NAME = "CP";
      ...
      if (BT_COMMAND_STR==":GR" || BT_COMMAND_STR==":gr"){
        Current_RA_DEC();
        Serial3.print(curr_RA_lz);
        Serial3.print("#");
        Serial.print(curr_RA_lz);
        Serial.print("#");
        BT_COMMAND_STR = "";
      }
      // :GD#  - Request DEC coordinate    
      if (BT_COMMAND_STR==":GD" || BT_COMMAND_STR==":gd"){
        Current_RA_DEC();
        Serial3.print(curr_DEC_lz);
        Serial3.print("#");
        Serial.print(curr_DEC_lz);
        Serial.print("#");        
        BT_COMMAND_STR = "";
      }
     // :Sr 07:08:52# - Set Target RA
      if (BT_COMMAND_STR.indexOf("Sr")>0 || BT_COMMAND_STR.indexOf("sr")>0){
     ...
     // :Sd +18Я12:30#  - Set Target DEC
      if (BT_COMMAND_STR.indexOf("Sd")>0 || BT_COMMAND_STR.indexOf("sd")>0){
        ...
        Serial3.print("1");
        Serial.print("Dec set");
      }
      // :MS#  -- Slew To Target RA and DEC
      if (BT_COMMAND_STR==":MS" || BT_COMMAND_STR==":ms"){
           ...
           calculateLST_HA();
            if (ALT > 0){
                ...
                Serial.print("Slew ok");   // 0 means that everything is OK
                ...
                Serial.print("1rDUINO Scope: Object Below Horizon! #");
            }
           drawMainScreen();
           BT_COMMAND_STR = "";
      }
      if (BT_COMMAND_STR=="Current"){
            Serial.println("Sending current info to BT");
        ...
        if (BT_COMMAND_STR.indexOf("SlewTo")>0){
           //Serial.println("BT SlewTo Command");
        ...
        if (BT_COMMAND_STR=="Status"){
          ...
          // You can add messages, but not remove!
           ...
           Serial3.print(_temp* 1.8 + 32, 0); //CtoF
           Serial3.print("F and humidity was "); //CtoF
