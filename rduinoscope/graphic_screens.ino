...
//    * CURRENT_SCREEN==8  - drawFilterScreen(); - previously not used
...
//
  ...
  int temp_N_selected_object_index = -1; //hold currently selected for indicate visible objects
  int temp_N_selected_object_set = -1;   //hold currently selected for indicate visible objects
...
void drawGPSScreen(){
  ...
  if (loaded_gps_from_SD == 0){
    tft.print("for Mount Sinai, NY");
  }
  else {
    tft.print("from last GPS pos");  
  }
...
//  tft.print(returnTime());
  ...
  for (int i=0; i<4; i++){
    for (int j=0; j<3; j++){
      if (kk==10){
        if (Summer_Time == 1){
          DrawButton(11,336,71,56, "SUMMER", btn_d_border, btn_l_border, btn_l_text, 1); //draw SUMMER button with bright background indicates currently summmer
        }else{
          DrawButton(11,336,71,56, "SUMMER", 0, btn_l_border, btn_l_text, 1); //draw SUMMMER button with black background indicates not currently summer
        }
      }else if (kk==11){
...
void setCP(){
  //set CP for use in restoring previously selected object
         OBJECT_NAME = "CP";
...
void save_currently_selected(){
     //save current object to be restored after drawing screen
 temp_N_selected_object_index = N_selected_object_index; //indicate visible objects
 temp_N_selected_object_set = N_selected_object_set; //indicate visible objects
}

void restore_currently_selected(){
    //restore previously selected object after a visibility check
    N_selected_object_index = temp_N_selected_object_index; //indicate visible objects
    N_selected_object_set = temp_N_selected_object_set; //indicate visible objects
    if (N_selected_object_index == -1) {
      setCP();
    }
    else if (N_selected_object_index == -2){
       //No object selected, only occurs prior to first object select
       OBJECT_NAME = "";
       OBJECT_DESCR = "";
       OBJECT_RA_M = 0;       
       OBJECT_DEC_D = 90;
      ...
      selectOBJECT_M(N_selected_object_index,N_selected_object_set);
      calculateLST_HA();
    }
}
...
if (OBJECT_NAME != "CP" && OBJECT_NAME != "" && IS_TRACKING == false && ALT >= 0){
    //tft.setTextColor(MsgBox_t);
    tft.setTextColor(MsgBox_t, btn_d_border); //MsgBox_bg);
    tft.println("*** SLEWING TO ***");
    tft.setTextColor(l_text);
}
else {
  tft.println("OBSERVING:");
...
//Check if object should be filtered based on the selection filters
bool should_be_filtered(String N_object_filter_in, String N_Object_type_in){
            if (N_object_filter_in == "GC" and N_Object_type_in != "Globular Cluster"){
            //M_NAME = "suppress";
            return true;
          }
          else if (N_object_filter_in == "OC" and 
                    N_Object_type_in != "Open Cluster" and
                    N_Object_type_in != "Asterism"
                    ){
             //M_NAME = "suppress";
             return true;
          }
          else if (N_object_filter_in == "Neb" and 
                    N_Object_type_in != "Nebula" and 
                    N_Object_type_in != "Planetary Nebula" and
                    N_Object_type_in != "Nebula/Open Cluster" and 
                    N_Object_type_in != "Thick Milky Way Patch" and 
                    N_Object_type_in != "Bright Nebula" and 
                    N_Object_type_in != "Supernova Remnant" 
                    ){
            //M_NAME = "suppress";
            return true;
          }
          else if (N_object_filter_in == "Glxy" and
                    N_Object_type_in != "Galaxy" and
                    N_Object_type_in != "Spiral Galaxy" and
                    N_Object_type_in != "Elliptical Galaxy" and
                    N_Object_type_in != "Irregular Galaxy"
                    ){
            return true; //M_NAME = "suppress";
          }
          else {
            return false;
          }
...
// Draw buttons to load CSVs
  ...
  DrawButton(91,360,60,35, N_object_filter, messie_btn, btn_l_border, btn_l_text, 2);
  
  
   if (LOAD_SELECTOR == 1){
     
    save_currently_selected(); //save current object to be restored after drawing screen

    Current_RA_DEC(); //force an update to RA and DEC global variables
    //float curr_RA_H, curr_RA_M, curr_RA_S, curr_DEC_D, curr_DEC_M, curr_DEC_S;
    Serial.println("scope current position:");
    Serial.print("RA ");
    Serial.print(curr_RA_H);
    Serial.print(", ");
    Serial.print(curr_RA_M);
    Serial.print(" DEC ");
    Serial.print(curr_DEC_D);
    Serial.print(", ");
    Serial.println(curr_DEC_M);
   ...
   int kk = MESS_PAGER*15;
      ...
      for (int i=0; i<5; i++){
          ...
          //detect end of object list
          if (M_NAME == "" ){
            //keeptrying = false;
            break;
          }

          //load object information for use in selection filtering
          selectOBJECT_M(kk,0);
          calculateLST_HA();
          ...
          Serial.print(curr_RA_H - OBJECT_RA_H + (curr_RA_M - OBJECT_RA_M)/60);
          //Serial.print(", ");
          //Serial.print(curr_RA_M - OBJECT_RA_M);
          Serial.print(" DEC ");
          Serial.println(curr_DEC_D - OBJECT_DEC_D + (curr_DEC_M - OBJECT_DEC_M)/60);
          //Serial.print(", ");
          //Serial.println(curr_DEC_M - OBJECT_DEC_M);


          //object filtering by position above horizon
          Serial.print("ALT= ");
          Serial.println(ALT);
          if (N_object_filter != "All" and ALT < ALT_threshold){
              M_NAME = "suppress";
             }
            
          //object filtering by type
          if (should_be_filtered(N_object_filter, N_Object_type)){
            M_NAME = "suppress";
          }


          //object filtering by magnitude
          if (N_visible_magnitude.toFloat() > N_magnitude_threshold){
            M_NAME = "suppress";
          }

          //display object or placeholder if suppressed
          if (M_NAME != "suppress") {
           //keeptrying = false;
           if (N_show_magnsize){
                    lower_line_text  =  N_visible_magnitude + ", " + N_object_size;
                  }
            
            DrawButton(((j*75)+9),((i*50)+85),71,45, M_NAME, messie_btn, 0, btn_l_text, 2, lower_line_text);  
          }
          else{
            DrawButton(((j*75)+9),((i*50)+85),71,45, "(" + String(kk + 1) + ")", DarkerGrey, 0, btn_l_text, 1);  
          }
    ...
    restore_currently_selected() ;//restore current object
    ...
    int ll = TREAS_PAGER*15;
      ...
      for (int i=0; i<5; i++){
          ...
          if (M_NAME != "suppress") {
            ...
            DrawButton(((j*75)+9),((i*50)+85),71,45, M_NAME, messie_btn, 0, btn_l_text, 1, lower_line_text);  
          }
          else{
            DrawButton(((j*75)+9),((i*50)+85),71,45, "(" + String(ll + 1) + ")", DarkerGrey, 0, btn_l_text, 1);  
          }
...
void drawFilterScreen(){
  ...
  tft.println("Magnitude");
   if (N_magnitude_threshold == 100){
      DrawButton(5,65,73,30, "All", btn_d_border, btn_l_border, btn_l_text, 2);
      DrawButton(83,65,73,30, "mag <6", 0, btn_l_border, btn_l_text, 2);
      DrawButton(161,65,73,30, "mag <4", 0, btn_l_border, btn_l_text, 2);      
   }else if (N_magnitude_threshold == 4){
      DrawButton(5,65,73,30, "All", 0, btn_l_border, btn_l_text, 2);
      DrawButton(83,65,73,30, "mag <6", 0, btn_l_border, btn_l_text, 2);
      DrawButton(161,65,73,30, "mag <4", btn_d_border, btn_l_border, btn_l_text, 2);
  }else if (N_magnitude_threshold == 6) {
      DrawButton(5,65,73,30, "All", 0, btn_l_border, btn_l_text, 2);
      DrawButton(83,65,73,30, "mag <6", btn_d_border, btn_l_border, btn_l_text, 2);
      DrawButton(161,65,73,30, "mag <4", 0, btn_l_border, btn_l_text, 2);
  }
   /*
  //tft.drawLine (5, 110, 235, 110, btn_l_text);
  ...
  */
  ...
  tft.setCursor(10, 165);
  tft.setTextSize(2);
  tft.print("Object Type");

    if (N_object_filter == "All"){
    DrawButton(5,185,40,30, "All", btn_d_border, btn_l_border, btn_l_text, 1);
  }else{
    DrawButton(5,185,40,30, "All", 0, btn_l_border, btn_l_text, 1);    
  }
  if (N_object_filter == "Vis"){
    DrawButton(48,185,35,30, "Vis", btn_d_border, btn_l_border, btn_l_text, 1);
  }else{
    DrawButton(48,185,35,30, "Vis", 0, btn_l_border, btn_l_text, 1);
  }
  if (N_object_filter == "GC"){
    DrawButton(86,185,35,30, "GC", btn_d_border, btn_l_border, btn_l_text, 1);
  }else{
    DrawButton(86,185,35,30, "GC", 0, btn_l_border, btn_l_text, 1);
  }
  if (N_object_filter == "OC"){
    DrawButton(124,185,35,30, "OC", btn_d_border, btn_l_border, btn_l_text, 1);
  }else{
    DrawButton(124,185,35,30, "OC", 0, btn_l_border, btn_l_text, 1);
  }
  if (N_object_filter == "Neb"){
    DrawButton(162,185,35,30, "Neb", btn_d_border, btn_l_border, btn_l_text, 1);
  }else{
    DrawButton(162,185,35,30, "Neb", 0, btn_l_border, btn_l_text, 1);
  }
  if (N_object_filter == "Glxy"){
    DrawButton(200,185,35,30, "Glxy", btn_d_border, btn_l_border, btn_l_text, 1);
  }else{
    DrawButton(200,185,35,30, "Glxy", 0, btn_l_border, btn_l_text, 1);
  }

  // Minimum Altitude
  tft.setCursor(10, 225);
  tft.setTextSize(2);
  tft.println("Declination threshold");
  if (ALT_threshold == 0){
      DrawButton(5,245,73,30, "Horizn", btn_d_border, btn_l_border, btn_l_text, 2);
      DrawButton(83,245,73,30, "30 deg", 0, btn_l_border, btn_l_text, 2);
  }else{
      DrawButton(5,245,73,30, "Horizn", 0, btn_l_border, btn_l_text, 2);
      DrawButton(83,245,73,30, "30 deg", btn_d_border, btn_l_border, btn_l_text, 2);    
  }
  /*
  // Sound On/Off
  tft.setCursor(10, 285);
...
void drawSTATScreen(){
  ...
  tft.setCursor(0, 40);
           ...
           tft.print(_Stemp * 1.8 + 32, 0); //CtoF
           tft.print("F and "); //CtoF
           tft.print(_Shumid, 0);
...
void considerDayNightMode(){
  boolean prev_night_mode = IS_NIGHTMODE;  
          //if (analogRead(A8) < 800){ //was A6
          //if (analogRead(daynight_pin) < 800){ //was A6
          //if (false)//temporarily disable 
          if (digitalRead(daynight_pin) == LOW)
          { 
              IS_NIGHTMODE = true;
          } else {
              IS_NIGHTMODE = false;
...
//  }
  ...
  if (MAIN_SCREEN_MENU == 0){
     if (IS_BT_MODE_ON == false){
      ...
      if (autoguiding_active){
        DrawButton( 161, 280, 78, 55, "GUIDE", messie_btn, btn_l_border, btn_l_text, 2);
        //DrawButton(91,360,60,35, N_object_filter, messie_btn, btn_l_border, btn_l_text, 2);
      }
      else if (IS_TRACKING == true){
